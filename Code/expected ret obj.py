# ========================================================================
# 
#    BASELINE MODEL WITH DYNAMIC THETA AND C0 C1 DATA COMBINED 29 day priority
# 
# ========================================================================

# ========================================================================
# FULL MONTHLY ANALYSIS WITH PROFESSOR'S TABLE OUTPUT
# Complete code with composition tracking and Post2025 format table
# FIXED VERSION - Corrected annualization and IQR calculations
# ========================================================================
...
print("FULL DATASET ANALYSIS - ALL MONTHS")
...
#spx_values = pd.read_csv("spx_values.csv")
...
# ============================================================================
# SELECT TRADING DAY: Priority 28-day (Friday), fallback 29-day (Thursday)
# ============================================================================

print("Selecting trading days with priority logic...")

# For each expiration month (id), determine which maturity to use
selected_data_list = []

for month_id in sorted(options_data['id'].unique()):
    month_subset = options_data[options_data['id'] == month_id]
    
    # Check availability for each maturity
    has_29 = (month_subset['Maturity'] == 29).any()
    has_28 = (month_subset['Maturity'] == 28).any()
    has_30 = (month_subset['Maturity'] == 30).any()
    
    if has_29:
        # Priority 1: Use 29-day options (Friday)
        selected = month_subset[month_subset['Maturity'] == 29].copy()
        print(f"Month {month_id}: Using 29-day options")
    elif has_28:
        # Priority 2: Fallback to 28-day options (Thursday)
        selected = month_subset[month_subset['Maturity'] == 28].copy()
        print(f"Month {month_id}: No 29-day available, using 28-day options")
    elif has_30:
        # Priority 3: Fallback to 30-day options (Wednesday)
        selected = month_subset[month_subset['Maturity'] == 30].copy()
        print(f"Month {month_id}: No 29 or 28-day available, using 30-day options")
    else:
        # No valid maturity found - skip this month
        print(f"Month {month_id}: WARNING - No 28, 29, or 30-day options available, skipping!")
        continue
...
lpm_results_list = []
performance_list = []
composition_list = []  # NEW: Track portfolio composition
positions_list = []
# ============================================================================
...
for month_id in unique_ids:
    ...
    try:
        ...
        T = month_data['Maturity'].iloc[0] / 365  # Use actual maturity from data
        ...
        # ====================================================================
        # GET DYNAMIC MINCER-ZARNOWITZ COEFFICIENTS
        # ====================================================================
        ...
        if len(mz_row) == 0:
            # Fallback: try to match by date
            mz_row = mz_coefficients[mz_coefficients['date'] == date]
            
            if len(mz_row) == 0:
                print(f"WARNING: No M-Z coefficients for month {month_id} (date: {date})")
                print(f"         Using sample median values")
        ...
        print(f"M-Z Coefficients: c0={c0:.4f}, c1={c1:.4f}", end="")
        if not pd.isna(n_obs):
            print(f" (n={int(n_obs)} obs)")
        else:
            print(" (using median)") 

        # ====================================================================
        # GET DYNAMIC SGT PARAMETERS (ADD THIS NEW SECTION!)
        # ====================================================================
        ...
        if len(sgt_row) == 0:
            ...
            if len(sgt_row) == 0:
                print(f"WARNING: No SGT parameters for month {month_id} (date: {date})")
                print(f"         Using sample median values")
            ...
            # If multiple rows for same ID, take the first one
            theta1 = sgt_row['theta1'].iloc[0]
            theta2 = sgt_row['theta2'].iloc[0]
            theta3 = sgt_row['theta3'].iloc[0]
        ...
        # ====================================================================
        # GENERATE SGT–BASED SCENARIOS (POST2025/BSZ25 METHODOLOGY)
        # ====================================================================
        
        # # Fixed SGT parameters (Post2025 median estimates)
        # theta1 = 2.07    # shape parameter (p)
        # theta2 = 5.0       # degrees of freedom (q)
        # theta3 = -0.26   # asymmetry parameter (lambda)
        ...
        # DYNAMIC bias correction for VIX using M-Z regression
        sigma_annual_corrected = c0 + c1 * VIX_decimal
        sigma_period = sigma_annual_corrected * np.sqrt(T)
        
        # CAPM drift
        gamma = 3.25
        #lambda_t = gamma * (sigma_annual_corrected ** 2)
        lambda_period = gamma * (sigma_period ** 2)
        #mu = (rf - q + lambda_t) * T
        mu = (rf - q) * T + lambda_period
        
        # BINOMIAL PROBABILITIES - Cox et al. (1979)
        from scipy.special import gammaln
        
        n_steps = n_scenarios - 1
        k = np.arange(n_scenarios)
        log_binom = gammaln(n_steps + 1) - gammaln(k + 1) - gammaln(n_steps - k + 1)
        ...
        if not np.all(keep_indices):
            ...
            print(f"  Truncated: {n_scenarios_original} → {n_scenarios} scenarios")
        
        # Cumulative probabilities and midpoints for quantile mapping
        cumulative_prob = np.cumsum(probs)
        u_vals = cumulative_prob - probs / 2  # Midpoint of each probability mass
        
        # Generate SGT quantiles using R
        ...
        c = np.concatenate([
            -(expected_payoffs - asks * (1 + rf)**T),
            -(bids * (1 + rf)**T - expected_payoffs)
        ])
        ...
        # Co-moment (SSD) constraints
        for j in range(n_scenarios):
            if cumulative_prob[j] > 0:
                weighted_payoff = np.zeros(n_options)
                for i in range(j + 1):
                    weight = probs[i] / cumulative_prob[j]
                    weighted_payoff += weight * payoff_matrix[i, :]
                
                constraint = np.concatenate([-weighted_payoff, weighted_payoff])
                A_ub.append(constraint)
                b_ub.append(0)
        
        # Net premium constraint
        net_premium_constraint = np.concatenate([asks, -bids])
        A_ub.append(net_premium_constraint)
        b_ub.append(0)
        ...
        alpha_vals = result.x[:n_options]
        beta_vals = result.x[n_options:]
        ...
        option_payoffs_by_scenario = np.zeros(n_scenarios)
        for j_scenario in range(n_scenarios):
            option_payoffs_by_scenario[j_scenario] = (
                np.sum(alpha_vals * payoff_matrix[j_scenario, :]) - 
                np.sum(beta_vals * payoff_matrix[j_scenario, :])
            )
        ...
        n_thresholds = 5000
        step_size = (max_return - min_return) / 4900
...
#performance_df.to_excel('performance_er.xlsx', index=False)      
